...
namespace DigitalRuby.WeatherMaker
{
    ...
    public class LightningLightParameters
    {
        ...
        [Tooltip("Light position, zero if auto")]
        ...
        [Tooltip("Light flicker intensity")]
        [Range(0.0f, 1.0f)]
        public float LightFlickerIntensity = 0.3f;

        [Tooltip("Light flicker scale")]
        [Range(0.0f, 1.0f)]
        public float LightFlickerScale = 0.01f;
        ...
        {
            get { return (LightColor.a > 0.0f && LightIntensityMultiplier > 0.0f && LightIntensity >= 0.01f && LightPercent >= 0.0000001f && LightRange > 0.01f); }
        }
    }
...
// Unity 4.X does not support glow or fade or elapsed time
        ...
        private void ProcessAllLightningParameters()
        {
            ...
            {
                int maxLightsForEachParameters = (int)Mathf.Ceil((float)MaximumLightsPerBatch / (float)dependencies.Parameters.Count);
                delay.Minimum = parameters.DelayRange.Minimum + parameters.Delay;
                delay.Maximum = parameters.DelayRange.Maximum + parameters.Delay;
                parameters.maxLights = (lights >= MaximumLightsPerBatch ? 0 : maxLightsForEachParameters);
                lights += maxLightsForEachParameters;
        ...
        private Transform RenderLightningBolt(LightningBoltQualitySetting quality, int generations, int startGroupIndex, int endGroupIndex, LightningBoltParameters parameters)
        {
            ...
            if (dependencies.ThreadState != null)
            {
                dependencies.ThreadState.AddActionForMainThread(() =>
                {
                    ...
                    if (lp != null)
                    {
                        CreateLightsForGroup(segmentGroups[startGroupIndex], parameters, lp, quality, parameters.maxLights);
                    }
                });
            }
            else
        ...
        private void CreateLightsForGroup(LightningBoltSegmentGroup group, LightningBoltParameters p, LightningLightParameters lp, LightningBoltQualitySetting quality, int maxLights)
        {
            if (lightCount == MaximumLightCount || maxLights <= 0 || group.Generation < p.Generations)
            {
                return;
            }
            ...
            int nthLight, int nthShadows, ref int maxLights, ref int nthShadowCounter)
        {
            ...
            {
                pos = (group.Segments[segmentIndex].Start + group.Segments[segmentIndex].End) * 0.5f;
            ...
            {
                if (lp.LightPosition == Vector3.zero)
                {
                    pos = (group.Segments[segmentIndex].Start + group.Segments[segmentIndex].End) * 0.5f;
                }
                else
                {
                    pos = lp.LightPosition;
                }
            }
            if (dependencies.UseWorldSpace)
            {
        ...
        private void UpdateLight(LightningLightParameters lp, IEnumerable<Light> lights, float delay, float peakStart, float peakEnd, float lifeTime)
        {
            ...
            Vector2 flicker = new Vector2(lp.LightPosition.x, lp.LightPosition.z) * Time.timeSinceLevelLoad * lp.LightFlickerScale;
            float noise = 1.0f + (lp.LightFlickerIntensity * ((2.0f * (Mathf.PerlinNoise(flicker.x, flicker.y) - 0.5f))));
            if (realElapsedTime >= peakStart)
            {
                if (realElapsedTime <= peakEnd)
                {
                    ...
                    {
                        l.intensity = lp.LightIntensity * lp.LightIntensityMultiplier * noise;
                    }
                }
                else
                {
                    ...
                    foreach (Light l in lights)
                    {
                        l.intensity = Mathf.Lerp(lp.LightIntensity * lp.LightIntensityMultiplier * noise, 0.0f, lerp);
                    }
                }
            }
            else
            {
                ...
                foreach (Light l in lights)
                {
                    l.intensity = Mathf.Lerp(0.0f, lp.LightIntensity * lp.LightIntensityMultiplier * noise, lerp);
                }
            }
