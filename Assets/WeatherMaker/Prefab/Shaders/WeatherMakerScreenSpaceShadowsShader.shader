...
Shader "WeatherMaker/WeatherMakerScreenSpaceShadows" {
Properties {
...
inline float3 computeCameraSpacePosFromDepthAndVSInfo(v2f i)
{
    ...
    float depth = lerp(WM_LINEAR_DEPTH_01(zdepth), zdepth, unity_OrthoParams.w);
#if defined(UNITY_REVERSED_Z)
    zdepth = 1 - zdepth;
#endif
...
#endif
    fixed4 cascadeWeights = GET_CASCADE_WEIGHTS_Z(wpos, vpos.z);
